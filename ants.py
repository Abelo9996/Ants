...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        colony -- The AntColony, used to access game state information.
        """
        dest = self.place.exit
        # Extra credit: Special handling for bee direction
        # BEGIN EC
        if not self.forward:
            dest = self.place.entrance
        # END EC
        if self.blocked():
            self.sting(self.place.ant)
        elif self.armor > 0 and dest:
            self.move_to(dest)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        val_range = 0
        while loc:
            if len(loc.bees) and loc.exit and (val_range >= self.min_range and val_range <= self.max_range):
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            location_current = self.place.exit
            while location_current: 
                loc = location_current.ant
                if loc: 
                    if loc.is_container:
                        if not loc.is_buffed:
                            loc.damage *= 2
                            loc.is_buffed = True
                        if loc.contained_ant:
                            if not loc.contained_ant.is_buffed:
                                loc.contained_ant.damage *= 2
                                loc.contained_ant.is_buffed = True
                    else:                                          
                        if not loc.is_buffed:
                            loc.damage *= 2
                            loc.is_buffed = True
                location_current = location_current.exit
        # END Problem 13
